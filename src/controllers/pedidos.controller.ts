...
export function deletePedido (req: Request, res: Response): Response { 
    ...
    const oldPedido = database.pedidos.find( x => pedido.id);
...
function aplica5(pedido: Pedido): boolean {
    let fechaLimite = new Date();
    let sumaImporte = 0;
    //usar reduce
    fechaLimite.setDate(fechaLimite.getDate() - 14);
    const a = database.pedidos.filter(x => x.idUsuario === pedido.idUsuario)
    .filter(x => x.fecha > fechaLimite).forEach((x => {
        sumaImporte = sumaImporte + x.importe;
    })); 
    return sumaImporte > 100;
}

function aplica15(pedido: Pedido): boolean {
    let fechaLimite = new Date();
    let sumaImporte = 0;
    //usar reduce
